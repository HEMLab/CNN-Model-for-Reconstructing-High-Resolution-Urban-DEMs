...
def upsample_filt_NN(size,factor):
    """
    Make a 2D bilinear kernel suitable for upsampling of the given (h, w) size.
    """
    tmp_size = factor
    rst = np.zeros((size,size))
    st_id = int((size - tmp_size) / 2)
    end_id = int(st_id+tmp_size)
    rst[st_id:end_id,st_id:end_id] = 1
    return rst
...
def interp(net, layers):
    ...
    for l in layers:
        ...
        net.params[l][0].data[...]=np.zeros(net.params[l][0].data.shape)
        net.params[l][0].data[range(m), range(k), :, :] = filt

def interp_NN(net, layers, factors):
    """
    Set weights of each layer in layers to nearest-neighbor kernels for interpolation.
    """
    for cnt,l in enumerate(layers):
        ...
        if h != w:
            print 'filters need to be square'
            raise
        filt = upsample_filt_NN(h,factors[cnt])
        net.params[l][0].data[...]=np.zeros(net.params[l][0].data.shape)
        net.params[l][0].data[range(m), range(k), :, :] = filt
