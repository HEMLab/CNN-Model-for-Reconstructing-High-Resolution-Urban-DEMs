...
class GeoDataLondonDataLayer(caffe.Layer):
    ...
    def reshape(self, bottom, top):
        ...
        for cnt in range(self.batch_size):
            #print cnt
            self.data['image'][cnt,0,:,:],self.data['image_gt_2x'][cnt,0,:,:],self.data['image_gt_4x'][cnt,0,:,:],self.data['image_gt_8x'][cnt,0,:,:] = self.load_data(self.indices[self.idx[cnt]])
    ...
    def load_data(self, idx):
        ...
        return im_crop_lr,im_crop_2x[-self.size_2x:,-self.size_2x:],im_crop_4x[-self.size_4x:,-self.size_4x:],im_crop_8x[-self.size_8x:,-self.size_8x:] #crop HR resolution to match the output size
